...
public class MapComponent extends JPanel implements TrafficSimObserver {
	...
	private void drawRoads(Graphics g) {
		for (Road r : _map.getRoads()) {
			...
			if (idx != -1 && r.equals(r.getDestinationJunction().getincomingRoadList().get(idx))) {
			...
			int roadColorValue = 200 - (int) (200.0 * Math.min(1.0,
					(double) r.getTotalContamination() / (1.0 + (double) r.getContaminationAlarmLimit())));
